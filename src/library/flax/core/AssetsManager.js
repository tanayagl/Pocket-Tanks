...
flax.AssetsManager = cc.Class.extend({
    ...
    addAssets: function (assetsFile) {
        ...
        //changing the content size of the MovieClip
        for (var mc in dict.mcs) {
            var mcs = dict.mcs[mc]
            var rectArray = mcs.rect.split(",");
            rectArray[2] = rectArray[2] / cc.director.getContentScaleFactor();
            rectArray[3] = rectArray[3] / cc.director.getContentScaleFactor();
            mcs.rect = rectArray.join(",");
            for (var instance in mcs["children"]) {
                var childInstance = mcs["children"][instance];
                var pipeArray = childInstance.frames.split("|");
                for (var i = 0; i < pipeArray.length; i++) {
                    if (pipeArray[i] != "null" && pipeArray[i] != "") {
                        var array = pipeArray[i].split(",");
                        array[0] = array[0] / cc.director.getContentScaleFactor();
                        array[1] = array[1] / cc.director.getContentScaleFactor();
                        pipeArray[i] = array.join(",");
                    }
                }
                childInstance.frames = pipeArray.join("|");
            }
        }


        //the min tool version this API needed
        var toolVersion = dict["metadata"]["flaxVersion"] || dict["metadata"]["version"];
